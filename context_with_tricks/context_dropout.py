...
def cnn_context(inputs, keep_prob=1.0,scope=None, activation_fn=tf.nn.relu):
	with tf.variable_scope(None, 'cnn_net', [inputs]):
		with slim.arg_scope([slim.conv2d],  stride=1, padding='SAME' ,weights_regularizer=slim.l2_regularizer(0.01)):
			...
			net9_drop = slim.dropout(net9, keep_prob, scope='net9_drop')
			net10 = slim.conv2d(net9_drop, 640, [3,1],rate=1,scope='Conv2d_net10_3x1')
			...
			net17_drop = slim.dropout(net17, keep_prob, scope='net17_drop')
			net18 = slim.conv2d(net17_drop, 320, [11,1],rate=1,scope='Conv2d_net18_3x1')
			net18_drop = slim.dropout(net18, keep_prob, scope='net18_drop')
			netout = slim.conv2d(net18_drop, 3, [1,1],rate=1,activation_fn=None,scope='Conv2d_netout')
	return netout
